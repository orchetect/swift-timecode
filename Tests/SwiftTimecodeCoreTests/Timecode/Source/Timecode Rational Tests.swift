...
@Suite
struct Timecode_Source_Rational_Tests {
    @Test(arguments: TimecodeFrameRate.allCases)
    func timecode_init_Rational_Exactly(frameRate: TimecodeFrameRate) throws {
        let tc = try Timecode(
    ...
    @Test(arguments: TimecodeFrameRate.allCases)
    func timecode_init_Rational(frameRate: TimecodeFrameRate) throws {
        // these rational fractions and timecodes are taken from actual FCP XML files as known truth

        switch frameRate {
        case .fps23_976:
            #expect(
                try Timecode(.rational(Fraction(335_335, 24000)), at: frameRate).components
                    == Timecode.Components(h: 00, m: 00, s: 13, f: 23)
            )
        case .fps24:
            #expect(
                try Timecode(.rational(Fraction(167_500, 12000)), at: frameRate).components
                    == Timecode.Components(h: 00, m: 00, s: 13, f: 23)
            )
        case .fps24_98:
        ...
        case .fps29_97: // same fraction is found in FCP XML for 29.97p and 29.97i video rates
            #expect(
                try Timecode(.rational(Fraction(838_838, 60000)), at: frameRate).components
                    == Timecode.Components(h: 00, m: 00, s: 13, f: 29)
            )
            #expect(
                try Timecode(.rational(Fraction(1_920_919, 30000)), at: frameRate).components
                    == Timecode.Components(h: 00, m: 01, s: 03, f: 29)
            )
        case .fps29_97d:
            #expect(
                try Timecode(.rational(Fraction(419_419, 30000)), at: frameRate).components
                    == Timecode.Components(h: 00, m: 00, s: 13, f: 29)
            )
            #expect(
                try Timecode(.rational(Fraction(1_918_917, 30000)), at: frameRate).components
                    == Timecode.Components(h: 00, m: 01, s: 03, f: 29)
            )
        case .fps30:
            #expect(
        ...
        case .fps59_94:
            #expect(
                try Timecode(.rational(Fraction(839_839, 60000)), at: frameRate).components
    ...
    @Test(arguments: TimecodeFrameRate.allCases)
    func timecode_rationalValue(frameRate: TimecodeFrameRate) throws {
    ...
    @Test
    func timecode_RationalValue_SpotCheck() throws {
        let tc = try Timecode(.components(h: 00, m: 00, s: 13, f: 29), at: .fps29_97d)
        #expect(tc.rationalValue.numerator == 419_419)
        #expect(tc.rationalValue.denominator == 30000)
    }

    @Test
    func timecode_RationalValue_Subframes() throws {
        let tc = try Timecode(
    ...
    @Test
    func timecode_RationalSubframes() throws {
    ...
    @Test
    func timecode_FrameCountOfRational() throws {
        // 00:00:01:11.56 @ 25i fps, 80sf base
        // this fraction is actually a little past 56 subframes
        // because it was from FCPXML where it was not on an exact subframe
        // FYI: when we convert it back to a fraction from timecode,
    ...
    @Test
    func timecode_FloatingFrameCountOfRational() throws {
        // 00:00:01:11.56 @ 25i fps, 80sf base
        // this fraction is actually a little past 56 subframes
        // because it was from FCPXML where it was not on an exact subframe
        // FYI: when we convert it back to a fraction from timecode,
