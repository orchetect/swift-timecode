...
@Suite
struct Timecode_Conversion_Tests {
    ...
    func converted_NewFrameRate(frameRate: TimecodeFrameRate) throws {
    ...
    @Test
    func converted_NewFrameRate_spotCheck() throws {
    ...
    /// Spot-check an example conversion.
    @Test
    func converted_NewFrameRate_NewSubFramesBaseA() throws {
        let convertedTC = try Timecode(
            .components(sf: 50),
            at: .fps30,
            base: .max100SubFrames
    ...
    /// Spot-check an example conversion.
    @Test
    func converted_NewFrameRate_NewSubFramesBaseB() throws {
        let convertedTC = try Timecode(
    ...
    /// Baseline check: Ensure conversion produces identical output if frame rates are equal.
    @Test(arguments: TimecodeFrameRate.allCases)
    func converted_NewFrameRate_PreservingValues(frameRate: TimecodeFrameRate) throws {
    ...
    @Test(arguments: TimecodeFrameRate.allCases)
    func converted_NewFrameRate_PreservingValues(sourceFrameRate: TimecodeFrameRate) throws {
    ...
    @Test
    func converted_NewFrameRate_PreservingValues_Conversion() throws {
        let convertedTC = try Timecode(
