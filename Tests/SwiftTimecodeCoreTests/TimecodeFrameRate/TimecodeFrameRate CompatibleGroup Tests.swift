...
@Suite
struct TimecodeFrameRate_CompatibleGroup_Tests {
    @Test(arguments: TimecodeFrameRate.allCases)
    func compatibleGroup_EnsureAllFrameRateCasesAreHandled(frameRate: TimecodeFrameRate) {
    ...
    func testCompatibleGroup_compatibleGroupRates(grouping: (key: TimecodeFrameRate.CompatibleGroup, value: [TimecodeFrameRate])) {
    ...
    @Test(arguments: TimecodeFrameRate.CompatibleGroup.table)
    func testCompatibleGroup_isCompatible(grouping: (key: TimecodeFrameRate.CompatibleGroup, value: [TimecodeFrameRate])) {
        let otherGroupingsRates = TimecodeFrameRate.CompatibleGroup.table
            .compactMap { $0.key != grouping.key ? $0 : nil }
            .reduce(into: []) { $0 += ($1.value) }
